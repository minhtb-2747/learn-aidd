...
/**
 * Uploads ONE composer image to the public `kudo-images` bucket — one call per
 * file, because `next.config.ts` caps action bodies at 6 MB.
 *
 * Runs as the signed-in user under Storage RLS (no service key), so the object
 * path is what the INSERT policy checks: `(storage.foldername(name))[1] =
 * auth.uid()::text`. The path is always built here from the session's
 * `user.id`, never accepted from the client.
 */
...
/**
 * Best-effort rollback for a submit whose objects landed but whose `kudos` row
 * did not. The `{auth.uid()}/...` prefix check mirrors the DELETE Storage
 * policy, so a tampered path is rejected here too, not only by RLS.
 */
