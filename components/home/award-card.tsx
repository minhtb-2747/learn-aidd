...
export default function AwardCard({
  ...
  return (
    <article className="flex w-full max-w-84 flex-col items-start gap-6 transition-transform duration-200 hover:-translate-y-1">
      <div className="relative flex aspect-square w-full items-center justify-center overflow-hidden rounded-3xl border-[0.955px] border-[#FFEA9E] shadow-[0_4px_4px_rgba(0,0,0,0.25),0_0_6px_#FAE287] transition-shadow duration-200 hover:shadow-[0_4px_16px_rgba(0,0,0,0.35),0_0_16px_#FAE287]">
        <Image
      ...
      <div className="flex flex-col items-start gap-1">
        <h3 className="text-2xl leading-8 text-[#FFEA9E]">{title}</h3>
        <p className="text-base leading-6 tracking-[0.5px] text-white">
          {description}
        </p>
        <a
          href={`/awards#${slug}`}
          className="mt-2 flex items-center gap-1 py-4 text-base leading-6 font-bold tracking-[0.15px] text-white transition-colors duration-200 hover:text-[#FFEA9E]"
        >
          {detailLabel}
